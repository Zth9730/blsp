...
class MinibatchKmeansClusterOneStep(torch.nn.Module):
    ...
    @torch.no_grad()
    @torch.jit.unused
    def _one_step(self, samples: torch.Tensor,
                  means: torch.Tensor) -> torch.Tensor:
        """ One step iter
        Args:
        ...
        distance = self.compute_distance(
            samples, means)  # [groups, n_samples, num_clusters]
        ...
        inertiais = selected_distances.sum()
        bins_count = local_batched_bincount(
            bins, self.num_clusters)  # [groups, n_samples]
        dist.all_reduce(bins_count, group=process_group)
        zeros_bins_mask = (bins_count == 0)
        bins_count = torch.where(zeros_bins_mask, 1, bins_count)
        new_means = bins.new_zeros(self.groups,
                                   self.num_clusters,
                                   self.dim,
                                   dtype=samples.dtype)
        new_means.scatter_add_(1,
                               ...
                               samples)
        new_means /= bins_count.unsqueeze(2)
        dist.all_reduce(new_means, group=process_group)

        if self.use_cosine_sim:
            new_means = torch.nn.functional.normalize(new_means, p=2, dim=-1)
        return torch.where((zeros_bins_mask).unsqueeze(2), means, new_means), inertiais
    ...
    def forward(self, input: torch.Tensor):
        ...
        if not self.is_initialized:
            ...
            means = self.means
        means, inertiais = self._one_step(
            input.unsqueeze(0),
            means,
        )
        self.means.copy_(means)
        return means, inertiais
...
def get_dataset(configs, data_list_file, partition, args):
    ...
    conf['shuffle'] = False
    conf['sort'] = False
    conf['cycle'] = configs.get('max_epoch', 100)
...
def main():
    ...
    with torch.no_grad():
        ...
        for (i, batch) in enumerate(dataloader):
            ...
            encoder_out, encoder_out_mask = embed_model(speech, speech_lens, 12)
            ...
            means, inertiais = kmeans_model(selected_samples)
            if rank == 0 and i % 100 == 0:
                print("iter: {} | rank {} | inertiais {} | means: {}".format(i, rank, inertiais, means))

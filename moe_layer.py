...
class MoELayer(nn.Module):
    ...
    def _forward_gate_token(self, x):
        ...
        num_tokens = F.one_hot(gate, self.num_experts).gt(0).sum(0) # 每一个gate有多少token
    ...
    def _forward_gate_sentence(self, x, attention_mask):
        ...
        order = gate.argsort(0)
        num_sentences = F.one_hot(gate, self.num_experts).gt(0).sum(0)  # 每一个gate有多少token
...
expert = torch.nn.Linear(512, 512)
moe_layer = MoELayer(hidden_size=512, num_experts=4, expert=expert, 
                     route_method='gate-token', vocab_size='100', hash_list=None)
x = torch.randn(10, 20, 512)
input_ids = None
attention_mask = None

x, balance_loss, gate_load = moe_layer(x, input_ids, attention_mask)
print(sum(p.numel() for p in moe_layer.parameters() if p.requires_grad))
print(x.shape, balance_loss, gate_load)
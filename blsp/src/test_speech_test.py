...
def process_dataset_multitask(batch, tokenizer):
    ...
    # ground_truth
    if batch["task"] == "SER":
        end_entity_value = tokenizer("<|endofentityvalue|>").input_ids[1:]
        ground_truth_ids = tokenizer(batch["ground_truth"]).input_ids[1:] + end_entity_value + [tokenizer.eos_token_id]
    elif batch["task"] == "MIC":
        instrument = tokenizer("<|instrument|>").input_ids[1:]
        ground_truth_ids = []
        for i in batch["ground_truth"].split(","):
            ground_truth_ids += instrument
            ground_truth_ids += tokenizer(i).input_ids[1:]
        ground_truth_ids += [tokenizer.eos_token_id]
    elif batch["task"] == "SED":
        event = tokenizer("<|event|>").input_ids[1:]
        ground_truth_ids = []
        for i in batch["ground_truth"].split(","):
            ground_truth_ids += event
            ground_truth_ids += tokenizer(i).input_ids[1:]
        ground_truth_ids += [tokenizer.eos_token_id]
    elif batch["task"] == "AQA":
        ...
        ground_truth_ids = aqa_ground_truth_ids_prefix + aqa_ground_truth_ids_suffix
    elif batch["task"] == "SV":
        speaker_tag = tokenizer("<|speaker_meta|>").input_ids[1:]
        ground_truth_ids = []
        for i in batch["ground_truth"].split(","):
            ground_truth_ids += speaker_tag
            ground_truth_ids += tokenizer(i).input_ids[1:]
        ground_truth_ids += [tokenizer.eos_token_id]
    elif batch["task"] == "IC":
        ground_truth_ids = tokenizer("<|scenario|>").input_ids[1:] + tokenizer(batch["ground_truth"].split(",")[-1]).input_ids[1:] + tokenizer("<|action|>").input_ids[1:] + tokenizer(batch["ground_truth"].split(",")[0]).input_ids[1:] + tokenizer("<|entities|>").input_ids[1:] + tokenizer(batch["ground_truth"].split(",")[1]).input_ids[1:] + [tokenizer.eos_token_id]
    else:
        ground_truth_ids = tokenizer(batch["ground_truth"]).input_ids[1:] + [tokenizer.eos_token_id]
    ...
    suffix_attention_mask += [1] * len(ground_truth_ids)
    if batch["task"] == "AQA":
        suffix_labels += [-100] * len(aqa_ground_truth_ids_prefix) + aqa_ground_truth_ids_suffix
    else:
        suffix_labels += ground_truth_ids
    #pdb.set_trace()
    #print("suffix id:", input_ids, suffix_input_ids, suffix_labels) 
    #response = tokenizer.decode(suffix_input_ids, skip_special_tokens=False)
    #print("decode id:", suffix_input_ids)
    #print("response for input and task:", batch["task"], response)
    batch["input_ids"] = input_ids
    batch["attention_mask"] = attention_mask
    batch["labels"] = labels
    batch["suffix_input_ids"] = suffix_input_ids
...
def load_speech_text_paired_dataset(dataroot="",
    manifest_files="",
    tokenizer=None,
    instruction="",
    num_proc=64,
...
def offline_process(
    ...
    text_tokenizer = AutoTokenizer.from_pretrained(lm_path,  trust_remote_code=True)
    text_tokenizer.add_special_tokens({"additional_special_tokens": SPECIA_TOKENS + [val for val in WLT.values()]})
